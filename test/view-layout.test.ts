...
describe("view-layout", () => {
  it("defaults to chat-only full width", () => {
    expect(DEFAULT_VIEW_STATE).toEqual({ chat: true, workflow: false, code: false })
  ...
  it("activates workflow and deactivates code", () => {
    const withCode = { chat: true, workflow: false, code: true }
    const next = toggleViewPanel(withCode, "workflow")
    expect(next).toEqual({ chat: true, workflow: true, code: false })
  })

  it("activates code and deactivates workflow", () => {
    const withWorkflow = { chat: true, workflow: true, code: false }
    const next = toggleViewPanel(withWorkflow, "code")
    expect(next).toEqual({ chat: true, workflow: false, code: true })
  })

  it("deactivates workflow when more than one panel is active", () => {
    const state = { chat: true, workflow: true, code: false }
    const next = toggleViewPanel(state, "workflow")
    expect(next).toEqual({ chat: true, workflow: false, code: false })
  })
