...
Requires `@executioncontrolprotocol/*@^0.14.0` from npm (or `pnpm run link:ecp` to a local build) so browser `core/compile` exports `compileHarnessArtifactSource` (used by the coding harness).
