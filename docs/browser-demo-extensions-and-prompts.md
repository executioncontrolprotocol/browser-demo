...
4. Runs workflows via **`ecp.run(manifest)`**, which invokes step capabilities (e.g. `@executioncontrolprotocol/demo.echo`).
...
flowchart TB
  ...
  Panels --> Run
  Run --> Echo["@executioncontrolprotocol/demo.echo"]
```
...
`createDemoAppEnvironment()` builds on `createBrowserDemoEnvironment("browser-demo-app")` from `@executioncontrolprotocol/browser`. The demo extension is **already bound** in that environment — no extra app-level bind is required.

- **`@executioncontrolprotocol/demo`** is registered via `registerBrowserDefaults()` and bound with `extension("@executioncontrolprotocol/demo").with({})`, so `@executioncontrolprotocol/demo.echo` (and other demo step capabilities) appear in `describe()` and validate for workflow steps.
...
`@executioncontrolprotocol/registry-control` allows namespaces: `@executioncontrolprotocol/demo`, `@executioncontrolprotocol/chrome-ai`, `@executioncontrolprotocol/openai`, `@executioncontrolprotocol/claude`, `@executioncontrolprotocol/browser`, `@customer/*`.
...
| ---------- | ----- | ------ |
| `@executioncontrolprotocol/demo.echo` | `{ value?: unknown }` | `{ echo: unknown }` |

Registered via `@executioncontrolprotocol/demo`; bound in the browser demo environment. Demo-generated workflows and the offline `generateText` stub reference `@executioncontrolprotocol/demo.echo` on the echo step.
...
- **Default:** fixed `@executioncontrolprotocol.workflow` TOON with one step `echo` → `@executioncontrolprotocol/demo.echo`.
...
| Execute | `ecp.run(manifest)` | Step `uses` e.g. `@executioncontrolprotocol/demo.echo` |
